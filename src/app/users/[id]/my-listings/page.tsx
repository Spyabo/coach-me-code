      ...
      <div className="container max-w-7xl mx-auto sm:px-6 lg:px- mt-20">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-2 ">
          {listings.map((data: getListingsResponse) => {
            return (
              <li
                className="container bg-gray-200 shadow-md rounded border-solid border-2 border-purple-600 flex flex-col gap-4 py-4 px-2 text-center md:flex-1 justify-between items-center"
                key={data._id}
              >
                <Image
                  className="rounded border-solid border-2 border-orange-600"
                  width={300}
                  height={300}
                  src={data.listing_image}
                  alt={data.listing_title}
                />
                <h3 className="font-bold text-lg flex flex-wrap items-center text-center">
                  Listing title: {data.listing_title}
                </h3>
                <p>Listing description: {data.listing_description}</p>
                <p>
                  Programming languages:{" "}
                  {data.programming_languages.map((item) => item)}
                </p>
